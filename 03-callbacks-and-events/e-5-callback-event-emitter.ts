      ...
      files.forEach(file => {
        ...
        stat(filePath, (err, stats) => {
          ...
          if (stats.isFile()) {
            ...
            emitter.emit('file', fileItem)
            return done(null, fileItem)
          }
          if (stats.isDirectory()) {
            emitter.emit('dir', filePath)
...
const emitter = findLargestFile(import.meta.dirname, (err, largest) => {
  if (err) {
...
emitter
  .on('file', () => { })
  .on('dir', () => { })
  .on('end', largest => console.log('end', largest))
  .on('error', () => { })
